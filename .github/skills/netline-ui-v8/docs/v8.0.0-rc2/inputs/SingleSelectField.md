...
> ✅ **RECOMMENDED**: This is the preferred component for single-value selection. It replaces the deprecated `Select` component.

## Overview

- **Category**: inputs
- **Base Library**: custom
- **Status**: ✅ **PREFERRED** - Use this instead of deprecated `Select`

## Description

A single select dropdown (combobox) field.

It is a wrapper around the `Autocomplete` component, which provides a more flexible and customizable select dropdown field.
**This is the recommended replacement for the deprecated `Select` component.**

### Key Advantages over `Select`

- Built on `Autocomplete` for better UX (type-ahead, filtering)
- Consistent option format with `{ id, label, value }` pattern
...
args: {
  ...
  play: async ({ args, canvasElement }) => {
    ...
    // Needed for the browser to render the defaultValue
    await sleep(1);

    expect(input).toBeInTheDocument();
    expect(input).toHaveValue(args.defaultValue);

    expect(canvas.getByLabelText('With default value')).toBeInTheDocument();
...
args: {
  ...
  play: async ({ canvasElement }) => {
    ...
    for (let i = 0; i < optionItems.length; i += 1) {
      expect(optionItems[i]).toHaveTextContent(options[i]);
    }
  },
  parameters: {
    viewport: { height: 480 },
...
args: {
  ...
  play: async ({ canvasElement }) => {
    ...
    for (let i = 0; i < optionItems.length; i += 1) {
      expect(optionItems[i]).toHaveTextContent(newOptions[i]);
    }
  },
  parameters: {
    viewport: { height: 480 },
...
args: {
  ...
  play: async ({ canvasElement }) => {
    ...
    expect(input).toHaveDisplayValue('Object1');
  },
}
```
